...
var vitamins=['tomato','salad','vegetables'];
	...
	$scope.getIngredients = function(url) {
		...
		$http({
			...
			function getBestOne() {
		  for(var i = 0; i < ingredients.length; i++){
		    if(protein.indexOf($scope.ingredients[i]) !== -1) {
		      return " proteins rich";
		    } else if(carbs.indexOf($scope.ingredients[i]) !== -1) {
		      return "carbs rich";
		    } else if(fats.indexOf($scope.ingredients[i]) !== -1) {
		      return "fats rich";
		    } else if(vitamins.indexOf($scope.ingredients[i]) !== -1) {
		      return "vitamins rich";
		    }
		  }

		}

	var food_response= getBestOne();
	$scope.food_var=food_response;
